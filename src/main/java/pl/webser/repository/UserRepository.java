...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query("SELECT CASE WHEN EXIST (SELECT u.emailAddress FROM User u WHERE u.emailAddress =?1) THEN true ELSE false END")
    ...
    void updateUserPasswordById(String password);

    @Modifying
    @Query("UPDATE User u SET u.aboutMeInfo = ?1 WHERE u.id = ?2")
    void updateUserAboutMeInfoById(String aboutMeInfo);
